...
public sealed class InventoryPartDisplayDriver : ContentPartDisplayDriver<InventoryPart>
{
    ...
    public override async Task<IDisplayResult> UpdateAsync(InventoryPart part, UpdatePartEditorContext context)
    {
        ...
        {
            context.Updater.ModelState.AddModelError("ProductPart.Sku", T["The Product SKU is missing."]);
        ...
        {
            part.Inventory.Add(item.Key, item.Value);
        }
        ...
        {
            var singleValue = part.Inventory.Values.Single();
            part.Inventory.Clear();
            part.Inventory.Add(currentSku, singleValue);
            part.InventoryKeys.Clear();
            part.InventoryKeys.Add(currentSku);
        }
        else if (skuChanged)
        {
            ...
            part.InventoryKeys.Clear();
            foreach (var key in newInventory.Keys)
            {
                part.InventoryKeys.Add(key);
            }
        }
    ...
    private static void BuildViewModel(InventoryPartViewModel model, InventoryPart part)
    {
        ...
        foreach (var item in part.FilterOutdatedEntries())
        {
            model.Inventory.Add(item.Key, item.Value);
        }
